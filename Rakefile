...
task :ocra_no_lzma, :version do |t, args|
  ...
  system("ocra --chdir-first --no-lzma --icon './data/icon.ico' --output 'Weightless Player#{args[:version].!=('') ? " #{args[:version]}" : ''}.exe' './bin/youtube_list' './lib/**/*' './data/*'")
end

desc 'ocra'
task :ocra, :version do |t, args|
  args.with_defaults(:version => '')
  system("ocra --chdir-first --icon './data/icon.ico' --output 'Weightless Player#{args[:version].!=('') ? " #{args[:version]}" : ''}.exe' './bin/youtube_list' './lib/**/*' './data/*'")
end
