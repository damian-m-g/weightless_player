...
task :ocra_no_lzma, :version do |t, args|
  ...
  system("ocra --chdir-first --no-lzma --output 'youtube_list#{args[:version].!=('') ? "_#{args[:version]}" : ''}.exe' './bin/youtube_list' './lib/**/*'")
end

desc 'ocra'
task :ocra, :version do |t, args|
  args.with_defaults(:version => '')
  system("ocra --chdir-first --output 'youtube_list#{args[:version].!=('') ? "_#{args[:version]}" : ''}.exe' './bin/youtube_list' './lib/**/*'")
end